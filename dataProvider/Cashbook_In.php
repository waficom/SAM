...
class Cashbook_In
{
    ...
    public function getCashbook_In(stdClass $params)
    {
        ...
        $sql = "select * from cashbook_in where co_id='$params->co_id' and cb_type='I' ORDER BY $orderx DESC";
        $this -> db -> setSQL($sql);
        $rows = array();
    ...
    public function getCashbook_In_Detail(stdClass $params)
    {
        ...
        $sql = "select A.*, B.coa_nama from cb_in_detail A
        left join coa B on A.co_id=B.co_id and A.account=B.coa_id where inv_code= '$params->inv_code' ORDER BY $orderx DESC";
        $this -> db -> setSQL($sql);
        $rows = array();
        foreach ($this->db->fetchRecords(PDO::FETCH_ASSOC) as $row)
        {
