...
class Cashbook_Bank_In
{
    ...
    public function updateCashbook_Bank_In_Detail(stdClass $params)
    {
        ...
        $data['timeedit'] = Time::getLocalTime('Y-m-d H:i:s');
        unset($data['id'],$data['inv_code']);
        $sql = $this -> db -> sqlBind($data, 'cb_bank_detail', 'U', array('inv_code' => $params->inv_code));
        $this -> db -> setSQL($sql);
        $this -> db -> execLog();
        return $params;
    }
    ...
    {
        $sql = "DELETE FROM cb_bank_detail WHERE inv_code = '$params->inv_code' and account = '$params->account'";
        $this -> db -> setSQL($sql);
        $this -> db -> execLog();
        return $params;
    }
